...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        geolocation_type = ContentType.objects.get(app_label="geolocation", model="uszip")
        for consumer in orm.Consumer.objects.filter(geolocation_id=None):
            consumer.geolocation_type = geolocation_type
            try:
                consumer.save()
            except ValidationError as error:
                # Saving checks for dupes, but bad data may already exist.
                dupes = orm.Consumer.objects.filter(email=consumer.email)
                for bad_consumer in dupes:
                    print('bad_consumer.id: %s' % bad_consumer.id)
                raise error
