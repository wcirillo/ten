...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for consumer in orm.Consumer.objects.filter(Q(is_bouncing_email=True) |Q(is_blacklisted=True) | Q(is_perm_optout=True)):
           if consumer.is_bouncing_email:
               consumer.is_emailable = False
               consumer.nomail_reason.add(1)
           if consumer.is_blacklisted:
               consumer.is_emailable = False
               consumer.nomail_reason.add(3)
           if consumer.is_perm_optout:
               consumer.is_emailable = False
               consumer.nomail_reason.add(2)
           consumer.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for consumer in orm.Consumer.objects.filter(is_emailable=False):
           if consumer.nomail_reason == 1:
               consmer.is_bouncing_email = True
           if consumer.nomail_reason == 3:
               consumer.is_blacklisted = True
           if consumer.nomail_reason == 2:
               consumer.is_perm_optout = True
           consumer.save()
