...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        bad_ids = orm['consumer.Consumer'].objects.filter(
                id__in=orm.AdRep.objects.all()
            ).filter(id__in=orm.AdRepLead.objects.all()
            ).values_list('id', flat=True)

        if bad_ids:
            LOG.info('Deleting %s leads; already are ad reps' % len(bad_ids))
        for id in bad_ids:
            ad_rep_lead = orm.AdRepLead.objects.get(id=id)
            LOG.info('Deleting lead %s' % ad_rep_lead.email)
            cursor = connection.cursor()
            cursor.execute("""
                DELETE FROM firestorm_adreplead
                WHERE consumer_ptr_id = %s;""", [id])
    ...
    models = {
        ...
        'coupon.coupon': {
            ...
            'expiration_date': ('django.db.models.fields.DateField', [], {'default': 'datetime.date(2012, 3, 11)', 'db_index': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'ecommerce.promotion': {
            ...
            'end_date': ('django.db.models.fields.DateField', [], {'default': 'datetime.date(2012, 12, 11)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'is_active': ('django.db.models.fields.BooleanField', [], {'default': 'True'}),
