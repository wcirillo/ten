...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """ move twitter_name from Site to TwitterAccount table """
        for site in orm.Site.objects.all():
            try:
                twitter_account = orm.TwitterAccount.objects.get(site=site)
            except orm.TwitterAccount.DoesNotExist:
                twitter_account = orm.TwitterAccount(site=site)
            if site.twitter_name and site.twitter_name != '':
                twitter_account.twitter_name = site.twitter_name
                twitter_account.password = 'Onlinecouponsareawesome1!'
                twitter_account.save()


    def backwards(self, orm):
        """ move twitter_name from TwitterAccount to Site table """
        for site in orm.Site.objects.all():
            if site.twitter_account.twitter_name and \
               site.twitter_account.twitter_name != '':
                site.twitter_name = site.twitter_account.twitter_name
                site.save()
