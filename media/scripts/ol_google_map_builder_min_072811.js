...
{var location_array=new Array();var loc_map=new Array();var layer_array=new Array();var whole_market_layer;var style_array=new Array();var selected_geoms_array=new Array();var file_prefix;marker_list={};loc_map[0]={};loc_map[1]={};var hover_control_id=0;return{build_location_map:function(loc_list,minimap)
...
{var bounds=new OpenLayers.Bounds();var this_bounds;var myLabel;var geom_vector_data;var zoom_level='market';var style_index;var geom_data=markets.replace(/['"]/g,'');var geom_array=geom_data.split('|');var initial_layer_index;var geom_unit;var fill_opacity=0.7;if(typeof visible=="undefined"){visible=true;}
...
{myLabel=geom_array[i][1];}
else
{myLabel='';}
style_index=i.toString().substring(i.toString().length-1);style_array[i]=new OpenLayers.StyleMap({"default":new OpenLayers.Style({pointRadius:"${type}",fillColor:color_list[style_index][0].toString(),strokeColor:color_list[style_index][1].toString(),strokeWidth:1.3,graphicZIndex:1,fillOpacity:fill_opacity,label:myLabel,fontColor:"#382282",fontSize:12,fontWeight:"600",labelAlign:"cm",labelXOffset:i,labelYOffset:i-2})});if(geom_array[i][0]!='')
{if(geom_array[i][2]!=undefined)
{myLabel=geom_array[i][2].toLowerCase();}
layer_array[initial_layer_index+i]=new OpenLayers.Layer.Vector(geom_array[i][1].toLowerCase(),{styleMap:style_array[i],mode:geom_unit,LID:myLabel,visibility:visible});geom_data1=geom_array[i][0];geom_vector_data=loc_map[0].read_wkt(geom_data1);if(mode=="whole_market")
...
{zoom_level='hawaii';}}
else if(this_bounds.toString().replace(/[^0-9,.]/g,'').toString().split(',')[0]<16600000&&this_bounds.toString().replace(/[^0-9,.]/g,'').toString().split(',')[2]>14960000&&this_bounds.toString().replace(/[^0-9,.]/g,'').toString().split(',')[2]<15700000)
{zoom_level='alaska';}
if(mode!="zip"){ol_map.custom_zoom(loc_map[0].map,bounds,zoom_level);if(!loc_map[0].map.getCenter()){loc_map[0].map.zoomToMaxExtent()}}
else{ol_map.display_layer($("#id_zip_postal").val());ol_map.adjust_zip_zone($("#id_zip_postal").val(),$("#slider-range-min").slider("value"));}},build_geoms_for_single_market:function(markets,interactive,visible,mode)
{ol_map.build_base_layer();ol_map.build_geom_layers(markets,interactive,visible,mode);},build_map_selector:function(interactive,visible,mode,site_prefix)
{var counties=document.getElementById('map_counties').value.replace(/['"]/g,'');file_prefix=site_prefix;ol_map.build_geoms_for_single_market(counties,interactive=interactive,visible=visible,mode=mode);loc_map[0].map.addLayers([whole_market_layer]);loc_map[0].map.setLayerIndex(loc_map[0].map.getLayersBy("name","whole_market")[0],1);},load_zip_geoms:function()
{for(var i=loc_map[0].map.layers.length-1;i>=0;i--)
{if(loc_map[0].map.layers[i].mode=='zip')
{ol_map.display_layer($("#id_zip_postal").val());ol_map.adjust_zip_zone($("#id_zip_postal").val(),$("#slider-range-min").slider("value"));return true;}}
alert('Need a loading msg give it 5 or 6 seconds to load ...');ol_map.load_doc(file_prefix+'-zip-geom-data.txt');},custom_zoom:function(map,bounds,type)
{if(typeof type=="undefined"){type="default";}
switch(type)
{case"alaska":myZoom=4;lonCenter=-163.58278125;latCenter=63.07486734225869;break;case"hawaii":myZoom=7;lonCenter=-157.8583333;latCenter=21.3069444;break;case"market":map.zoomToExtent(bounds);return;case"continent":default:myZoom=3;lonCenter=-110.683333;latCenter=50.033333;break;}
...
{var bounds=new OpenLayers.Bounds();var this_bounds;var geom_data=document.getElementById('id_geom').value.replace(/['"]/g,'');var geom_array=geom_data.split('|');var layer_array=new Array();var geom_vector_data;document.getElementById('loadingmap').style.visibility='hidden';for(var i=0;i<geom_array.length-1;i++)
...
{var temp_point;var line;var dist;central_zip=loc_map[0].map.getLayersBy("name",zip)[0];centroid=central_zip.features[0].geometry.getCentroid();for(var i=loc_map[0].map.layers.length-1;i>=0;i--)
{if(loc_map[0].map.layers[i].mode=='zip')
...
{ol_map.hide_layer(loc_map[0].map.layers[i].name);}}}},clear_zip_selections:function()
...
{selected='';for(n=0;n<selected_geoms_array.length;n++)
{selected=selected+selected_geoms_array[n][0]+', '}},load_doc:function(file)
{var xmlhttp;var file_path;if(file.indexOf('state_geoms') > -1){file_path = "/media/data/"}else{file_path = "/media/dynamic/map-data/"}if(window.XMLHttpRequest)
...
else if(file=='market-geom-markers.txt')
{marker_list=xmlhttp.responseText;ol_map.build_markets_map();}
else
{ol_map.build_geom_layers(xmlhttp.responseText,false,false,"zip");}}}
...
function county_click(obj)
{if(obj.checked==true)
{ol_map.display_layer(obj.value);}
else
{ol_map.hide_layer(obj.value);}}