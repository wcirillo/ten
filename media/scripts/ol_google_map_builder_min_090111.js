...
{ol_map.build_base_layer();ol_map.build_geom_layers(markets,interactive,visible,mode);},build_map_selector:function(interactive,visible,mode)
{var counties=document.getElementById('map_counties').value.replace(/['"]/g,'');ol_map.build_geoms_for_single_market(counties,interactive=interactive,visible=visible,mode=mode);loc_map[0].map.addLayers([whole_market_layer]);loc_map[0].map.setLayerIndex(loc_map[0].map.getLayersBy("name","whole_market")[0],1);},custom_zoom:function(map,bounds,type)
