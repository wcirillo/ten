...
else{ol_map.build_location_map(loc_list);map_id=1;hover_control_id=1;div_map_id='minimap';}
...
{div_id='displaymap';}if(typeof map_id=='undefined')
{map_id=0;}ol_map.market_map_setup(loc_map[map_id]);OpenLayers.ImgPath='/media/images/'
...
else{nested_array[i][2]='';}}}return nested_array},setMarkerToCenter:function(marker)
...
{switch(color_index){case 1:border_color=""
...
{zoom_level='hawaii';}
else if(loc_map[0].map.getLayersByName('anchorage').length+loc_map[0].map.getLayersByName('aleutians west').length>0)
{zoom_level='alaska';}
...
{ol_map.build_base_layer();ol_map.build_geom_layers(markets,interactive,visible,mode);},build_market_map: function(markets){options_dict = {minZoomLevel: 0, maxZoomLevel: 10};ol_map.build_geoms_for_single_market(markets, false);},build_state_map:function(markets,interactive,visible,mode)
...
{if(typeof type=="undefined"){type="default";}switch(type){case"alaska":myZoom=4;lonCenter=-152.5341796875;latCenter=62.257014469733924;break;case"hawaii":myZoom=7;lonCenter=-157.8583333;latCenter=21.3069444;break;case"market":map.zoomToExtent(bounds);return;case"continent":default:myZoom=3;lonCenter=-110.683333;latCenter=50.033333;break;}
...
{unique_val=false;break;}}if(unique_val)
...
return true;},clear_zip_selections:function(){for(n=0;n<selected_geoms_array.length;n++)
...
{file_path="/media/data/"}else{file_path="/media/dynamic/map-data/"}
if(window.XMLHttpRequest){xmlhttp=new XMLHttpRequest();}
else{xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}
xmlhttp.onreadystatechange=function()
{if(xmlhttp.readyState==4&&xmlhttp.status==200)
{if(file.indexOf('state_geoms')>-1)
{document.getElementById("id_geom").value=xmlhttp.responseText;ol_map.build_country_map();}else
...
{if(layer_bounds==null){layer_bounds=marker_layers[i].getDataExtent();}
else{layer_bounds.extend(marker_layers[i].getDataExtent());}}
return layer_bounds;}function place_markers(map)
