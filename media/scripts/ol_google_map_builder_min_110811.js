...
if(mode=="whole_market")
{
        ...
        if(geom_array[i][0]!='')
            {
                ...
                layer_array[initial_layer_index+i]=new OpenLayers.Layer.Vector(layer_name,{styleMap:style_array[i],mode:geom_unit,LID:market_link,visibility:visible});/*dennis visibility to visibility:visible*/
    ...
    if(interactive==true)
        {
        var highlightCtrl=new OpenLayers.Control.SelectFeature(layer_array,{hover:true,highlightOnly:true,renderIntent:"temporary"});
        var selectCtrl=new OpenLayers.Control.SelectFeature(layer_array,{clickout:true,click:true,single:true,onSelect:function(e){window.location="/"+e.layer.LID.toString().replace(/\s/g,"").replace(' ','-')+'/';}});
...
adjust_zip_zone:function(zip,miles)
{
    ...
    for(var i=loc_map[0].map.layers.length-1;i>=0;i--)
    {
        if(loc_map[0].map.layers[i].mode == 'zip')
        {
            ...
            {
                ol_map.display_layer(loc_map[0].map.layers[i].name, true);
            }
